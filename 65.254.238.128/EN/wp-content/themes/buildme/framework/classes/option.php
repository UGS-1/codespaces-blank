...
class VP_Option
{
	...
	public function __construct(array $configs)
	{
		...
		extract($test = array_merge(array(
			...
			'layout'                => 'fixed',
			'page_title'            => esc_attr__( 'Vafpress Options', 'vp_textdomain' ),
			'menu_label'            => esc_attr__( 'Vafpress Options', 'vp_textdomain' ),
			'priority'              => 10,
		), $configs));

		// check and set required configs
		if(isset($option_key)) $this->set_option_key($option_key);
		else throw new Exception(esc_attr__( 'Option Key is required', 'vp_textdomain' ), 1);
		if(isset($template)) $this->set_template($template);
		else throw new Exception(esc_attr__( 'Template Array/File is required', 'vp_textdomain' ), 1);
		if(isset($page_slug)) $this->set_page_slug($page_slug);
		else throw new Exception(esc_attr__( 'Page Slug is required', 'vp_textdomain' ), 1);
	...
	{
		VP_WP_Util::admin_notice(esc_attr__("Development Mode is Active, options' values won't be saved into database.", 'vp_textdomain'), false);
	}
	...
	function ajax_import_option()
	{
		...
		if($result['status'])
		{
			...
			{
				$result['status']  = false;
				$result['message'] = esc_attr__("Can not be empty", 'vp_textdomain');
			}
			else
			{
				...
				{
					$result['status']  = false;
					$result['message'] = esc_attr__("Invalid data", 'vp_textdomain');
				}
			}
		}
	...
	function ajax_export_option()
	{
		...
		if($result['status'])
		{
			...
			$result = array(
				'status' => true,
				'message'=> esc_attr__("Successful", 'vp_textdomain'),
				'option' => $sr_options,
			);
		}
	...
	function vp_verify_nonce()
	{
		...
		{
			$result['status']  = true;
			$result['message'] = esc_attr__("Successful", 'vp_textdomain');	
		}
		else
		{
			$result['status']  = false;
			$result['message'] = esc_attr__("Unverified Access.", 'vp_textdomain');
		}
		return $result;
	}
	...
	public function init_options_set()
	{
		...
		if( is_string($this->get_template()) and is_file($this->get_template()) )
			...
			throw new Exception(esc_attr__( 'Invalid template supplied', 'vp_textdomain' ), 1);
		...
		if( $this->use_util_menu() )
		{
			...
			$util_menu->set_title(esc_attr__('Utility', 'vp_textdomain'));
			...
			$restore_section->set_title(esc_attr__('Restore Default', 'vp_textdomain'));
			...
			$exim_section->set_title(esc_attr__('Export/Import', 'vp_textdomain'));
