...
define( 'OZY_THEMENAME', 'BUILDME' ); //used in TGM Plugin Activation
...
define( 'OZY_NOTIFIER_THEME_FOLDER_NAME', 'buildme' );
define( 'OZY_NOTIFIER_XML_FILE', 'http://s3-eu-west-1.amazonaws.com/themeversion/buildme.xml' );
...
$theme_options = new VP_Option(array(
	...
	'option_key' 			=> 'vpt_ozy_buildme_option', // options key in db, required
	...
	'page_title' 			=> esc_attr__( 'Theme Options', 'vp_textdomain' ), // page title
	'menu_label' 			=> esc_attr__( 'Theme Options', 'vp_textdomain' ), // menu label
));
...
/**
* Add featured image thumbnail to Project Listing page on admin
*/

function ozy_add_img_column($columns) {
    $columns['ozy_featured_img'] = esc_attr__('Featured Image', 'vp_textdomain');
    return $columns;
}
add_filter('manage_ozy_project_posts_columns', 'ozy_add_img_column');

function ozy_manage_img_column($column, $post_id) {
    if( $column === 'ozy_featured_img' ) {
        echo get_the_post_thumbnail($post_id, 'thumbnail');
		return;
    }
}
add_filter('manage_ozy_project_posts_custom_column', 'ozy_manage_img_column', 10, 2);
