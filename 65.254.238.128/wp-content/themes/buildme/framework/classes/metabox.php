...
class VP_Metabox extends WPAlchemy_MetaBox
{
	...
	function __construct($arr)
	{
		...
		if( $this->is_dev_mode )
		{
			$this->title = esc_attr__('[Development Mode] ', 'vp_textdomain') . $this->title;
		}
	...
	function _render_repeating_group($group)
	{
		...
		foreach ($group['groups'] as $g)
		{
			...
			$html .= '<div class="vp-wpa-group-heading"><a href="#" class="vp-wpa-group-title">' . $icon . $group['title'] . '</a><a href="#" class="dodelete vp-wpa-group-remove" title="'. esc_attr__('Remove', 'vp_textdomain') .'"><i class="fa fa-times"></i> '. esc_attr__('Remove', 'vp_textdomain') .'</a></div>';
			$html .= '<div class="vp-controls' . ((!$is_first) ? ' vp-hide' : '') . '">';
		...
		$html .= '<a href="#" class="button button-large docopy-' . $name . '">'. esc_attr__('Add More', 'vp_textdomain') . ' ' . $group['title'] . '</a>';
		$html .= '</div>';
