...
class VP_Option_Control_Set
{
	...
	public function save($option_key)
	{
		...
		if(update_option($option_key, $opt))
		{
			...
			$result['message'] = esc_attr__('Saving successful', 'vp_textdomain');
		...
		else
		{
			...
			if($changed)
			{
				...
				$result['message'] = esc_attr__('Saving failed', 'vp_textdomain');
			}
			else
			{
				$result['status']  = true;
				$result['code']    = self::SAVE_NOCHANGES;
				$result['message'] = esc_attr__('No changes made', 'vp_textdomain');
			}
		}
