<?php
	class Ozy_myHelper{
		...
		*/		
		...
		function get_custom_font($font_name) { //, $font_weight = 'normal', $font_style = 'normal'
			if($font_name) {
				...
				if(isset($font->ID)) {
					$font_grp = vp_metabox('ozy_buildme_meta_font.ozy_buildme_meta_font_group', null, $font->ID);
					if(is_array($font_grp) && count($font_grp) > 0) {						
						foreach($font_grp as $fnt) {
							$id = $fnt['ozy_buildme_meta_font_id'];
							if(!in_array($id, $this->active_custom_fonts)) {
								$fnt_arr = array();
								$fnt_arr['title'] 	= $font->post_title;
								$fnt_arr['id'] 		= $id;
								$fnt_arr['eot'] 	= $fnt['ozy_buildme_meta_font_eot'];
								$fnt_arr['woff'] 	= $fnt['ozy_buildme_meta_font_woff'];
								$fnt_arr['ttf'] 	= $fnt['ozy_buildme_meta_font_ttf'];
								$fnt_arr['svg'] 	= $fnt['ozy_buildme_meta_font_svg'];
								$fnt_arr['weight'] 	= $fnt['ozy_buildme_meta_font_weight'];//$font_weight;
								$fnt_arr['style'] 	= $fnt['ozy_buildme_meta_font_style'];//$font_style;
		...
		*/		
		...
		{
			$background_slider_image_arr = vp_metabox('ozy_buildme_meta_page.ozy_buildme_meta_page_background_group.0.ozy_buildme_meta_page_background_slider_group');
	...
	jQuery.supersized({
		...
		slides : [';
			...
			foreach($background_slider_image_arr as $img) {
				$script .= $virgul . "{image:'" . esc_url($img['ozy_buildme_meta_page_background_slider_image']) . "', title:'', url:''}";
				$virgul = ",";
			}
$script .= '],
		progress_bar : 0,
		...
		*/
		...
		/**
		* project_unite_gallery
		*
		* Creates a slider from image ids whic used in [gallery] shortcode
		*
		* @hq - Boolean, to check if slider for blog post or full slider
		*/
		function project_unite_gallery($hq = false) {
			$slider_img_size = 'blog';
			if($hq) {
				$slider_img_size = 'full';			
			}
			...
			if(count($image_ids_arr)>0){
				foreach ( $image_ids_arr as $id ) {
					$small_image = wp_get_attachment_image_src( $id, 'thumbnail' );
					$large_image  = get_post($id);
					//$image_o .= '<img alt="'. esc_attr($large_image->post_title) .'" src="'. esc_url($small_image[0]) .'" data-image="'. esc_url($large_image->guid) .'" data-description="'. esc_attr($large_image->post_excerpt) .'">' . PHP_EOL;
					$image_o .= '<img src="'. esc_url($small_image[0]) .'" data-image="'. esc_url($large_image->guid) .'" data-description="'. esc_attr($large_image->post_excerpt) .'">' . PHP_EOL;
				}
				
				return '<div id="unite-gallery-'. rand(100,100000) .'" class="unite-gallery" style="display:none;">' . PHP_EOL . $image_o . PHP_EOL .'</div>';				
			}
			return '';
		}		
		...
		*/
		function newer_older_post_navigation_post($padding=false) {
            ...
            <div class="newer-older<?php echo $padding ? ' newer-older-blog' : '' ?>">
                <?php 
                    previous_post_link('%link', esc_attr__('&larr; Previous Post', 'vp_textdomain')); 
                    if((int)ozy_get_option('page_blog_list_page_id')>0) {
                        echo '<a href="'. get_permalink(ozy_get_option('page_blog_list_page_id')) .'">' . esc_attr__('All Posts', 'vp_textdomain') . '</a>';
                    }
                    next_post_link('%link', esc_attr__('Next Post &rarr;', 'vp_textdomain')) 
                ?>
            </div><!--.newer-older-->               
            <?php			
		}	
		...
		function ozy_masonry_blog_date_comment_box() {
			global $post;
			echo '<div class="post-meta">';								
			echo '<span class="m content-font">'; the_time('M'); echo '</span>';
			echo '<span class="d content-font">'; the_time('d'); echo '</span>';
			echo '<span class="y content-font">'; the_time('Y'); echo '</span>';
			echo '<span class="c content-font"><span class="n">'; comments_number(esc_attr__('NO', 'vp_textdomain'), '1', '%'); echo '</span><span class="t">'; esc_attr_e('COMMENTS', 'vp_textdomain'); echo '</span></span>';
			echo '</div><!--#post-meta-->';
		}		
	}
?>