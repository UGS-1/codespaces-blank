		...
		init: function() {
			...
			$( '.sf-menu' ).each( function( i ) {
				...
				var x 			= $( this ),
					...
					indicator2 	= x.data( 'indicator2' ) || ( x.hasClass( 'offcanvas_menu' ) ? 'fa fa-angle-right' : '' );
				...
				x.codevzMenu({
					...
					onInit: function() {
						...
						$( '.menu-item-has-children', this ).each( function() {
							...
							if ( ! en.find( '.cz_indicator' ).length ) {

								if ( $( '.cz_menu_subtitle', en ).length ) {
									$( '.cz_menu_subtitle', en ).before( '<i class="cz_indicator"></i>' );
								} else {
									en.find( '> a, > h6' ).append( '<i class="cz_indicator"></i>' );
								}

							}

							if ( indicator || indicator2 ) {
								$( '.cz_indicator', en ).addClass( indicator );
								$( 'ul .cz_indicator', en ).addClass( indicator2 );
							}
		...
		menu_anchor: function() {
			...
			if ( mLink.length ) {
				...
				mLink.off( 'click.anchor' ).on( 'click.anchor', function( e ) {
					...
					if ( $( this ).not( 'a[href*="#top"],.cz_no_anchor,.cz_no_anchor a, .vc_general, .cz_no_anchor a, .cz_lrpr a, .wc-tabs a, .cz_edit_popup_link, .page-numbers a, #cancel-comment-reply-link, .vc_carousel-control, [data-vc-container],.comment-form-rating a,.sm2-bar-ui a' ).length ) {
		...
		*/
		offCanvas: function( selector, click ) {
			...
			if( ! area.hasClass( 'xtra-reload-js' ) ) {
				...
				// reInit codevz plus.
				Codevz_Plus.init();
				if ( typeof Codevz_Plus.working_hours_line != 'undefined' ) {
