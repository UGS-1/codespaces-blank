		...
		public static function get_row_element( $i, $m = [] ) {
			...
			if ( $elm === 'logo' || $elm === 'logo_2' ) {
				...
				if ( $logo ) {
					...
					if ( $sizes && ! self::contains( $logo, '.svg' ) ) {
				...
				// Lazyload logo.
				echo class_exists( 'Codevz_Plus' ) ? Codevz_Plus::lazyload( $logo_html ) : $logo_html;
				...
				if ( $element_id === 'menu_header_4' && ( $mobile_menu_social || $mobile_menu_text || self::$preview ) ) {
					...
					echo $mobile_menu_social ? Codevz_Plus::social() : '';
				...
				if ( ! empty( $i[ 'social_type' ] ) ) {
					...
					if ( $i[ 'social_type' ] === 'popup' ) {
						...
						echo do_shortcode( '[cz_popup id_popup="xtra-social-popup" icon="fa czico-198-cancel" sk_popup="' . wp_kses_post( $container ) . '" sk_icon="color:#fff;"]' . wp_kses_post( $social ) . '[/cz_popup]' );
				...
				echo '<div class="outer_search" style="' . esc_attr( $outer_style ) . '"><div class="search" style="' . esc_attr( $form_style ) . '">'; ?>
					<form method="get" action="<?php echo self::$home_url; ?>" autocomplete="off">
						...
						<input name="order" type="hidden" value="<?php echo empty( $i['search_order'] ) ? '' : esc_attr( $i['search_order'] ); ?>" />
						<input name="orderby" type="hidden" value="<?php echo empty( $i['search_orderby'] ) ? '' : esc_attr( $i['search_orderby'] ); ?>" />
						...
						<input name="search_post_icon" type="hidden" value="<?php echo empty( $i['search_post_icon'] ) ? '' : esc_attr( $i['search_post_icon'] ); ?>" />
						<input name="sk_search_post_icon" type="hidden" value="<?php echo empty( $i['sk_search_post_icon'] ) ? '' : esc_attr( $i['sk_search_post_icon'] ); ?>" />
						
						<?php if ( defined( 'ICL_LANGUAGE_CODE' ) ) { ?>
							<input name="lang" type="hidden" value="<?php echo( ICL_LANGUAGE_CODE ); ?>" />
						<?php } ?>
		...
		public static function generate_page( $page = '' ) {
			...
			if ( $is_404 ) {
				...
				if ( have_posts() ) {
					...
					if ( $page !== 'page' && ! $blank && ( $title === '1' || $title === '2' || $title === '8' ) ) {
						...
						echo '<' . esc_attr( $page_title_tag ) . ' class="xtra-post-title section_title">' . get_the_title() . '<span class="xtra-post-title-date"><a href="' . self::$home_url . get_post_time( 'Y/m/d' ) . '"><time datetime="' . esc_attr( get_the_date( 'c' ) ) . '" itemprop="datePublished"><i class="far fa-clock mr8"></i>' . esc_html( get_the_date() ) . '</time></a></span></' . esc_attr( $page_title_tag ) . '>';
			...
			// Archive posts
				...
				if ( $template === 'x' && $custom_template && $custom_template !== 'none' ) {
					...
					if ( self::$preview ) {
						$prefix = ( ! $cpt || $cpt === 'post' ) ? 'blog' : $cpt;
						echo '<i class="xtra-section-focus fas fa-cog" data-section="' . esc_attr( $prefix ) . '_settings"></i>';
						echo '<i class="xtra-section-focus xtra-section-focus-second fas fa-paint-brush" data-section="' . esc_attr( $prefix ) . '_styles"></i>';
					}
