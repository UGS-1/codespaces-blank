		...
		$( '.cz_popup_modal' ).each( function( i, x ) {
			...
			var $this 		= $( x ),
				...
				showPopup 	= function() {
					...
					$this.find( '.cz_overlay' ).css( 'background', $this.data( 'overlay-bg' ) );
			...
			if ( wpb ) {
				...
				// Delete popup
				$( "#" + popupID + " .cz_close_popup, #cz_close_popup, .cz_overlay, a[href*='#cz_close_popup']" ).off();
				$( '.vc_control-btn-delete', parentX ).on('click', function() {
					$( '.edit_' + popupID, parent.document.body ).closest( 'li' ).remove();
				});
			}

			// Open popup.
			$( "a[href*='#" + popupID + "']" ).on( 'click', function( e ) {

				// Move popup to footer.
				if ( popup.length && ! $this.closest( '.vc_cz_popup' ).length ) {
