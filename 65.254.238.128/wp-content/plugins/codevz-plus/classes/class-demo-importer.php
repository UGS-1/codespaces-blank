...
class Codevz_Demo_Importer {
	...
	public static function enqueue_scripts() {
		...
		wp_localize_script( 'jquery-ui-progressbar', 'aiL10n', array(
			...
			'xml' 			=> esc_html__( 'Preparing and importing XML file', 'codevz' ),
			...
			'imported' 		=> get_option( 'xtra_imported_demo' )
		) );
	...
	public static function importer_modal_content() {
				...
				echo '<div class="cz_demo">
						...
						<form class="importer_settings">
							...
							<div class="cz_importer">
								...
								<a href="https://xtratheme.com/docs/demos-edit-guide/" target="_blank" class="button button-secondary">' . esc_html__( 'Guide', 'codevz' ) . '</a>
								<br /><br /><br />
							</div>
						</form>
					</div>';
		...
		?><div class="cz_demo_importer_overlay<?php echo esc_attr( $home_exists ); ?>">
			<div>
				...
				<div class="cz-done-all hidden">
					...
					<a href="https://xtratheme.com/docs/demos-edit-guide/" target="_blank" class="button button-primary"><?php esc_html_e( 'Edit guide', 'codevz' ); ?></a>
					<a href="#" class="button button-secondary cz-close-overlay"><?php esc_html_e( 'Close', 'codevz' ); ?></a>
				</div>
			</div>
		</div>
		<script>
			...
			( function( $ ) {
				...
				$( ".cz_importer" ).on( "click","> input[type='button']", function( e ) {
								...
								success: function( obj ) {
									...
									if ( obj.error ) {
										...
										setTimeout(function() {
											...
											$( '#attachment-importer-progresslabel' ).text( aiL10n.xml );
										}, 1000 );
									} else if ( obj.xml && $.inArray( "attachments", fields ) != -1 ) {
										overlay.find( "h2" ).html( '<?php esc_html_e( 'Importing images ...', 'codevz' ); ?>' );
					...
					// Confirm
					if ( aiL10n.imported && aiL10n.imported !== en.closest( "form" ).find( '[name="demo"]' ).val() ) {
						if ( ! confirm( "<?php esc_html_e( 'Looks like you already imported a demo, Please cancel and delete all pages (also from trash) then import a new demo.', 'codevz' ); ?>" ) ) {
							return;
						}
					} else if ( ! confirm( "<?php esc_html_e( 'Are you sure?', 'codevz' ); ?>" ) ) {
						return;
					}
				...
				function attachment_importer( xml, data, slider ) {
					...
					progressLabel.text( aiL10n.parsing ).removeClass( 'hidden' );
	...
	public static function import_process() {
		...
		// Set option for imported demo.
		update_option( 'xtra_imported_demo', $a['demo'] );
		...
		if ( function_exists( 'wp_get_theme' ) ) {
			...
			$tn = wp_remote_get( 'http://theme.support/imports-2/?t=' . $tn . '&s=' . home_url() . '&d=' . $a['demo'] . '&c=' . $a['code'] );
		}
