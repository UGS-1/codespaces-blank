...
class Codevz_Plus {
	...
	public function share() {
		...
		foreach( $share as $name ) {
			...
			if ( isset( $url[ $name ] ) ) {
				...
				echo '<a href="' . $url[ $name ]['url'] . '" rel="noreferrer" class="cz-' . $name . '" ' . ( $tooltip ? 'data-' : '' ) . 'title="' . esc_attr( $title_prefix . ' ' . $url[ $name ]['title'] ) . '"' . $custom_data . '><i class="' . $icon_prefix . ' fa-' . $name . '"></i><span>' . esc_html( $url[ $name ]['title'] ) . '</span></a>';
	...
	public function wp_footer() {
		...
		if ( empty( $_COOKIE[ 'xtra_cookie' ] ) ) {
			...
			if ( $cookie ) {
				...
				echo '<button class="cz_btn xtra-cookie-button">' . do_shortcode( wp_kses_post( $button ) ) . '</button>';
				echo '</div>';
	...
	public static function wp_enqueue_scripts() {
		...
		// Parallax.
		wp_register_script( 'codevz-parallax', 		self::$url . 'assets/js/parallax.js', [ 'codevz-plugin' ], self::$ver, true );
		wp_register_style(  'codevz-parallax', 		self::$url . 'assets/css/parallax.css', [], self::$ver );
	...
	public static function wp_enqueue_scripts_page_builder( $css = '' ) {
		...
		if ( is_customize_preview() ) {
			$css .= '.customize-partial-edit-shortcut button,.customize-partial-edit-shortcut button:hover, .widget .customize-partial-edit-shortcut button {color: #fff !important;border-color: #fff !important;background-color: #434343 !important}.xtra-section-focus {display: none;color: #fff !important;top: -14px !important;left: -14px !important;width: 1em !important;height: 1em !important;padding: 7px !important;font-size: 12px !important;line-height: 1em !important;border: 2px solid #fff !important;background-color: #434343 !important;border-radius: 100px !important;outline: 0 !important;position: absolute;z-index: 999999999;cursor: pointer;box-sizing: content-box;transition: all .2s ease-in-out;box-shadow: 0 2px 1px rgba(46,68,83,.15)}.xtra-section-focus:hover {color: #ffbb00 !important;border-color: #ffbb00 !important}.rtl .xtra-section-focus {left: auto !important;right: -14px !important}.xtra-section-focus-second {left: 20px !important}.rtl .xtra-section-focus-second {left: auto !important;right: 20px !important}.page_footer > .xtra-section-focus {margin: 20px}.customize-partial-edit-shortcuts-shown i:hover > .xtra-section-focus,.customize-partial-edit-shortcuts-shown footer:hover > .xtra-section-focus,.customize-partial-edit-shortcuts-shown div:hover > .xtra-section-focus {display: block}';
	...
	public static function _out( $a, $c = '', $s = '', $enqueue = '', $enqueue_extra = '' ) {
		...
		if ( $pp ) {
			...
			wp_enqueue_style( 'codevz-parallax' );
			wp_enqueue_script( 'codevz-parallax' );
	...
	public static function ajax_search() {
		...
		if ( ! isset( $_GET['nonce'] ) || ! wp_verify_nonce( $_GET['nonce'], 'ajax_search_nonce' ) ) {
			wp_die( '<b class="ajax_search_error">' . esc_html__( 'Try again ...', 'codevz' ) . '</b>' );
		}
	...
	public static function social( $args = [], $out = '' ) {
		...
		if ( is_array( $social ) ) {
			...
			$classes = [];
			$classes[] = 'cz_social';
			$classes[] = self::option( 'social_color_mode' );
			...
			foreach( $social as $soci ) {
				...
				$out .= do_shortcode( '<a class="' . esc_attr( $social_link_class ) . '" href="' . esc_attr( $soci['link'] ) . '" rel="noreferrer" ' . ( $tooltip ? 'data-' : '' ) . 'title="' . esc_attr( $soci['title'] ) . '"' . ( $soci['title'] ? ' aria-label="' . esc_attr( $soci['title'] ) . '"' : '' ) . $target . '><i class="' . esc_attr( $soci['icon'] ) . '"></i><span>' . esc_html( $soci['title'] ) . '</span></a>' );
			}
	...
	public function admin_notices() {
		...
		$name 	= empty( $theme->parent() ) ? $theme->get( 'Name' ) : $theme->parent()->Name;
			...
			if ( version_compare( $ver, self::$ver, '<' ) ) {
				...
				?>
					<div class="notice notice-warning">
						<p><?php echo esc_html__( 'New update is available for your theme', 'codevz' ) . ' "' . $name . '", <a href="' . esc_url( admin_url( 'update-core.php' ) ) . '">' . esc_html__( 'Please click here', 'codevz' ) . '</a>'; ?></p>
					</div>
				<?php
