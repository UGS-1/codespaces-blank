		...
		en.off().on( 'click', function( e ) {
		  ...
		  if ( ! mTitle.find( '.fa-refresh' ).length ) {
			...
			mTitle.append( '<i class="fa fa-refresh" data-title="Reset"></i>' ).find( '.fa-refresh' ).on( 'click', function() {
				...
				if ( confirm( 'Are you sure you want reset this StyleKit?' ) ) {
		  ...
		  // Responsive buttons
		  !mTitle.find( '.fa-mobile-alt' ).length && mTitle.append( '<i class="fas fa-mobile-alt" data-title="On Mobile"></i>' );
		  !mTitle.find( '.fa-tablet-alt' ).length && mTitle.append( '<i class="fas fa-tablet-alt" data-title="On Tablet"></i>' );
		  !mTitle.find( '.fa-desktop' ).length && mTitle.append( '<i class="fas fa-desktop cz_sk_active_tab" data-title="On Desktop"></i>' );
		  ...
		  mTitle.find( '.fa-file-o' ).addClass( 'hide' ).off().on( 'click', function() {
		  	...
		  	localStorage.setItem( 'sk_desktop', par.find( '[name="' + name + '"]' ).val() );
		  	localStorage.setItem( 'sk_tablet', par.find( '[name="' + nameT + '"]' ).val() );
		  	localStorage.setItem( 'sk_mobile', par.find( '[name="' + nameM + '"]' ).val() );
		  	localStorage.setItem( 'sk_hover', par.find( '[name="' + nameH + '"]' ).val() );
		  ...
		  mTitle.find( '.fa-files-o' ).addClass( 'hide' ).off().on( 'click', function() {
			...
			if ( confirm( 'Are you sure you want paste on this StyleKit?' ) ) {

				mTitle.find( '.fa-desktop' ).addClass( 'cz_sk_active_tab' ).siblings().removeClass( 'cz_sk_active_tab' );

				var desktop = localStorage.getItem( 'sk_desktop' );
				desktop && par.find( '[name="' + name + '"]' ).val( desktop ).trigger( 'change' ).next( 'a' ).trigger( 'click' );

				var tablet = localStorage.getItem( 'sk_tablet' );
				tablet && par.find( '[name="' + nameT + '"]' ).val( tablet ).trigger( 'change' );

				var mobile = localStorage.getItem( 'sk_mobile' );
				mobile && par.find( '[name="' + nameM + '"]' ).val( mobile ).trigger( 'change' );

				var hover = localStorage.getItem( 'sk_hover' );
